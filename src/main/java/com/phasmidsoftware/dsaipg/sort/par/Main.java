...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            FileOutputStream fis = new FileOutputStream("C:/Users/adity/Desktop/Spring 2025/PSA/DSAIPG/result.csv");
    ...
    private static void processCommand(String x, String y) {
        if (x.equalsIgnoreCase("N")) {
            ...
            System.out.println("Current common pool parallelism: " + parallelismLevel);
        } else if (x.equalsIgnoreCase("-cutoff")) {
            try {
                int newCutoff = Integer.parseInt(y);
                ParSort.cutoff = newCutoff;
                System.out.println("Cutoff set to: " + newCutoff);
            } catch (NumberFormatException e) {
                System.err.println("Invalid cutoff value provided. Using default.");
            }
        } else {
            System.out.println("Unknown command or not implemented: " + x);
        }
    }
    ...
    private static void performSortingTests() {
        Random random = new Random();
        ...
        for (int size : sizes) {
            ...
            for (int j = 50; j < 100; j++) {
                ...
                timeList.add(totalTimes / 10);
                System.out.println("Array Size: " + size + ", Cutoff: " + ParSort.cutoff + ", Average Time: " + (totalTimes / 10) + "ms");
            }
        }
    }
}